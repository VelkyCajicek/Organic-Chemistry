import re
from hydrocarbon_derivatives import HydrocarbonDerivativeList
...
def extractValues(listElement : str, residuesList : list) -> None: # Void that changes the list
    ...
    for i in range(0, len(HydrocarbonDerivativeList)): # Note to self: Thank fuck I already figured out this logic 
        if(HydrocarbonDerivativeList[i].preffix == listElement): # Although I may want to make this better in the future
            for index in positions: 
                if(residuesList[int(index) - 1] == HydrocarbonDerivativeList[i].functionalGroup): 
                    residuesList[int(index) - 1] = [residuesList[int(index) - 1]] 
                    residuesList[int(index) - 1].append(HydrocarbonDerivativeList[i].functionalGroup)
                elif(type(residuesList[int(index) - 1]) is list): 
                    residuesList[int(index) - 1].append(HydrocarbonDerivativeList[i].functionalGroup)
                else:
                    residuesList[int(index) - 1] = HydrocarbonDerivativeList[i].functionalGroup 
...
if __name__ == "__main__":
    testString = "4-chlor-hepta-1,2-dien"
